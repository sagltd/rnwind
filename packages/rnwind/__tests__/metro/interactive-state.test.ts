...
describe('transformAst: interactive variant wiring', () => {
  it('literal with active: token replaces the tag with _ib and packs the spec into _rw', () => {
    const out = run(`const V: any = () => null; export default () => <V className="bg-sky-500 active:bg-sky-700" />`)
  ...
  it('forwards user-supplied onPressIn untouched — _ib chains handlers internally', () => {
    const out = run(
      `const V: any = () => null; export default ({ onPressIn }: any) => <V className="active:bg-sky-700" onPressIn={onPressIn} />`,
    )
  ...
  it('forwards user-supplied onFocus untouched', () => {
    const out = run(
      `const V: any = () => null; export default ({ handleFocus }: any) => <V className="focus:opacity-50" onFocus={handleFocus} />`,
    )
  ...
  it('non-interactive literal keeps the inline `style={_l(...)}` shape — no _ib swap', () => {
    const out = run(`const V: any = () => null; export default () => <V className="flex-1 bg-white" />`)
  ...
  it('preserves existing style prop as _rw.us so the runtime can merge it into the resolved array', () => {
    const out = run(
      `const V: any = () => null; export default ({ outer }: any) => <V className="active:bg-sky-700" style={outer} />`,
    )
